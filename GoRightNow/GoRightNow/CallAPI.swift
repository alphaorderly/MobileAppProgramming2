        ...
        case .success:
            if let jsonData = json.value {
                for getData in jsonData.data {
                    getCountries.append(GoRightNowModel.Country(name: getData.country_nm, iso_alp2: getData.country_iso_alp2, immigInfo: "no data", flagImageURL: getData.flag_download_url, alarmLevel: getData.alarm_lvl ?? 0))
                }
            } else {
                ...
                if let jsonData = json.value {
                    for getData in jsonData.data {
                        ...
                        if flag == false { // 현재 단계에서는 데이터 테스트 용으로 사용
                            getCountries.append(GoRightNowModel.Country(name: getData.country_nm, iso_alp2: getData.country_iso_alp2, immigInfo: getData.txt_origin_cn, flagImageURL: "null", alarmLevel: -1))
                        }
                    }
                } else {
